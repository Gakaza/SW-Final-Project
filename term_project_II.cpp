...
//---------------------------------------------------------------
//Start of classification funtions w.r.t the attributes 0 ~ 19
//---------------------------------------------------------------
// 0 - age (numeric)
void ageClass(int &tempAge, char (&tempAttribute)[20][30], int (&numPerAge)[2][5], int &num_Age0, int &num_Age1){
	tempAge = atoi(tempAttribute[0]);
	if(tempAge < 30){
...
void jobClass(char (&tempAttribute)[20][30], int (&numPerJob)[2][11], int &num_job0, int &num_job1){
...
void maritalClass(char (&tempAttribute)[20][30], int (&numPerMat)[2][3], int &num_Mat0, int &num_Mat1){
...
void eduClass(char (&tempAttribute)[20][30], int (&numPerEdu)[2][7], int &num_Edu0, int &num_Edu1){
...
void defaultClass(char (&tempAttribute)[20][30], int (&numPerDefault)[2][2], int &num_Def0, int &num_Def1){
...
void houseClass(char (&tempAttribute)[20][30], int (&numPerHouse)[2][2], int &num_House0, int &num_House1){
...
void loanClass(char (&tempAttribute)[20][30], int (&numPerLoan)[2][2], int &num_Loan0, int &num_Loan1){
...
void contactClass(char (&tempAttribute)[20][30], int (&numPerCon)[2][2], int &num_Con0, int &num_Con1){
...
void monthClass(char (&tempAttribute)[20][30], int (&numPerMonth)[2][12], int &num_Month0, int &num_Month1){
...
void dayClass(char (&tempAttribute)[20][30], int (&numPerDay)[2][5], int &num_Day0, int &num_Day1){
...
void tempCampaignClass(int &tempCampaign, char (&tempAttribute)[20][30], int (&numPerCampaign)[2][3], int &num_Campaign0, int &num_Campaign1){	
//int tempCampaign, numPerCampaign[2][3] = {0}, num_Campaign0 = 0, num_Campaign1 = 0;
	tempCampaign = atoi(tempAttribute[10]);
...
void tempPDaysClass(int &tempPDays, char (&tempAttribute)[20][30], int (&numPerPDays)[2][5], int &num_PDays0, int &num_PDays1){		
	//int tempPDays, numPerPDays[2][5] = {0}, num_PDays0 = 0, num_PDays1 = 0;
	tempPDays = atoi(tempAttribute[11]);
...
//			- intervals: 0, (not 0)
void tempPreviousClass(int &tempPrevious, char (&tempAttribute)[20][30], int (&numPerPrevious)[2][2], int &num_Previous0, int &num_Previous1){			
	//int tempPrevious, numPerPrevious[2][2] = {0}, num_Previous0 = 0, num_Previous1 = 0;
	tempPrevious = atoi(tempAttribute[12]);
	if(tempPrevious == 0){
		if(strcmp(tempAttribute[19],"no") == 0){
			numPerPrevious[0][0]++;			
			num_Previous0++;
...
void numPerPoutcomeClass(char (&tempAttribute)[20][30], int (&numPerPoutcome)[2][3], int &num_Poutcome0, int &num_Poutcome1){			
...
void tempEmpClass(float &tempEmp, char (&tempAttribute)[20][30], int (&numPerEmp)[2][6], int &num_Emp0, int &num_Emp1){			
	//float tempEmp; int numPerEmp[2][6] = {0}, num_Emp0 = 0, num_Emp1 = 0;
	tempEmp = atof(tempAttribute[14]);
	if(tempEmp >= 1.00){
		if(strcmp(tempAttribute[19],"no") == 0){
			numPerEmp[0][0]++;			
			num_Emp0++;
...
void tempPriceClass(float &tempPrice, char (&tempAttribute)[20][30], int (&numPerPrice)[2][5], int &num_Price0, int &num_Price1){
	//float tempPrice; int numPerPrice[2][5] = {0}, num_Price0 = 0, num_Price1 = 0;
	tempPrice = atof(tempAttribute[15]);
	if(tempPrice >= 94.45){
		if(strcmp(tempAttribute[19],"no") == 0){
			numPerPrice[0][0]++;			
			num_Price0++;
...
void tempConfClass(float &tempConf, char (&tempAttribute)[20][30], int (&numPerConf)[2][6], int &num_Conf0, int &num_Conf1){
	//float tempConf; int numPerConf[2][6] = {0}, num_Conf0 = 0, num_Conf1 = 0;
	tempConf = atof(tempAttribute[16]);
	if(tempConf >= -30.0){
		if(strcmp(tempAttribute[19],"no") == 0){
			numPerConf[0][0]++;			
			num_Conf0++;
...
void tempEuribor3mClass(float &tempEuribor3m, char (&tempAttribute)[20][30], int (&numPerEuribor3m)[2][5], int &num_Euribor3m0, int &num_Euribor3m1){
	//float tempEuribor3m; int numPerEuribor3m[2][5] = {0}, num_Euribor3m0 = 0, num_Euribor3m1 = 0;
	tempEuribor3m = atof(tempAttribute[17]);
...
void tempNrClass(float &tempNr, char (&tempAttribute)[20][30], int (&numPerNr)[2][2], int &num_Nr0, int &num_Nr1){
	//int tempNr, numPerNr[2][2] = {0}, num_Nr0 = 0, num_Nr1 = 0;
	tempNr = atof(tempAttribute[18]);
	if(tempNr > 5200.0){
		if(strcmp(tempAttribute[19],"no") == 0){
			numPerNr[0][0]++;			
			num_Nr0++;
...
void numPerOutputClass(char (&tempAttribute)[20][30], int (&numPerOutput)[2]){	
...
//---------------------------------------------------------------
//End of classification funtions w.r.t the attributes 0 ~ 19
//---------------------------------------------------------------
...
int main(){
	...
	double CorrRate = 0;	//to test how many I predict correct.
	...
	int numPerPoutcome[2][3] = {0}, num_Poutcome0 = 0, num_Poutcome1 = 0;
	float EmpPercentage[2][6] = {0}, tempEmp;
	...
	float NoOutput_Percentage = 0, YesOutput_Percentage = 0;
						...
						, tempAttribute[17], tempAttribute[18], tempAttribute[19]) != EOF)				
		{
			...
			*/
			...
			/*10 - campaign: number of contacts performed during this campaign and for this client (numeric, includes last contact) 
					- 3 intervals: (0 ~ 3), (4~6), (7+)	*/
			//int tempCampaign, numPerCampaign[2][3] = {0}, num_Campaign0 = 0, num_Campaign1 = 0;
			tempCampaignClass(tempCampaign, tempAttribute, numPerCampaign, num_Campaign0, num_Campaign1);
			
			
			//11 - pdays: number of days that passed by after the client was last contacted from a previous campaign (numeric; 999 means client was not previously contacted) 
			//			- 4 intervals: (0~11), (12~20), (21 ~ 27), (999) 
			//int tempPDays, numPerPDays[2][5] = {0}, num_PDays0 = 0, num_PDays1 = 0;
			tempPDaysClass(tempPDays, tempAttribute, numPerPDays, num_PDays0, num_PDays1);
			
			
			//12 - previous: number of contacts performed before this campaign and for this client (numeric) 
			//			- intervals: 0, (not 0)
			//int tempPrevious, numPerPrevious[2][2] = {0}, num_Previous0 = 0, num_Previous1 = 0;
			tempPreviousClass(tempPrevious, tempAttribute, numPerPrevious, num_Previous0, num_Previous1);
			...
			//			- 4 intervals: (x>1.0), (-1.0<= x < 1.0) , (-2.0<= x <-1.0) , (x < -2.0)
			//float tempEmp; int numPerEmp[2][6] = {0}, num_Emp0 = 0, num_Emp1 = 0;
			tempEmpClass(tempEmp, tempAttribute, numPerEmp, num_Emp0, num_Emp1);
			
			//15 - cons.price.idx: consumer price index - monthly indicator (numeric)
			//			- 3 intervals: (x>94.45), (92.8 <= x < 94.45), (x < 92.8)
			//float tempPrice; int numPerPrice[2][5] = {0}, num_Price0 = 0, num_Price1 = 0;
			tempPriceClass(tempPrice, tempAttribute, numPerPrice, num_Price0, num_Price1);
			
			//16 - cons.conf.idx: consumer confidence index - monthly indicator (numeric) 
			//			- 4 intervals: (x>-30.0), (-35.0<= x < -30.0), (-45.0< = x < -35.0), ( x < -45.0)
			//float tempConf; int numPerConf[2][6] = {0}, num_Conf0 = 0, num_Conf1 = 0;
			tempConfClass(tempConf, tempAttribute, numPerConf, num_Conf0, num_Conf1);
			
			//May 30th.
			//17 - euribor3m: euribor 3 month rate - daily indicator (numeric) 
			//			- 4 intervals: (x > 4.86), ( 4.0 <= x < 4.86), ( 1.0 <= x < 4.0), , ( x < 1.0)
			//float tempEuribor3m; int numPerEuribor3m[2][5] = {0}, num_Euribor3m0 = 0, num_Euribor3m1 = 0;
			tempEuribor3mClass(tempEuribor3m, tempAttribute, numPerEuribor3m, num_Euribor3m0, num_Euribor3m1);
			
			//18 - nr.employed: number of employees - quarterly indicator (numeric) 
			//			- 2 intervals: (x > 5200.0), (x <= 5200.0)
			//int tempNr, numPerNr[2][2] = {0}, num_Nr0 = 0, num_Nr1 = 0;
			tempNrClass(tempNr, tempAttribute, numPerNr, num_Nr0, num_Nr1);
